...
try{
$stmt = $conexion->prepare('SELECT nota FROM cursa WHERE asignatura_id = ? AND alumno_id= ?');
 
$stmt->bind_param('ii',$_SESSION['asignatura'], $_SESSION['alumno']);
$stmt->execute();
$stmt->store_result();
...
try{
$stmt = $conexion->prepare('SELECT nombre,apellido FROM alumno WHERE matricula= ?');
 
$stmt->bind_param('i',$_SESSION['alumno']);
$stmt->execute();
$stmt->store_result();
...
<body class="loggedin">
    ...
    echo ' <div class="formulario"> <form action="http://127.0.0.1/version4/Sistema_Calificaciones/backend/procesar_nota.php" method="post">
...
?>
    ...
    <div class=foto2 style="background-image: url('http://127.0.0.1/version4/Sistema_Calificaciones/images/2.png');">
